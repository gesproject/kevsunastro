import type { ShaderDefinition } from "../shaderField";

/**
 * "Waves" -- supplied verbatim by the human's design reference for the Shows
 * section backdrop, adapted from Paper Shaders via the 21st.dev Shader
 * Builder (same provenance as the retired moss shader). Ported unchanged
 * into this project's shared vanilla WebGL engine (shaderField.ts).
 *
 * Engine deviations, both inherited from shaderField.ts's fixed uniform
 * contract (the same constraints plasma.ts and moss.ts shipped under):
 * - u_cursor is forced to presence 0, so the source preset's pointer-glow
 *   mode never runs.
 * - u_space is zeroed, so the preset's static offset (0.16, 0.05) is not
 *   applied. The composition reads correctly without it; seed and rotation
 *   carry the framing instead.
 */
...
void main() {
  ...
  p *= u_scale;
  // Field transform: rotate, pan, slow drift. (Cursor push and static
  // offset are not exposed by the shared engine; see header.)
...
// Human-supplied preset values, verbatim: near-black indigo ground through
// violet to muted lavender (colours 4-8 unused; colorCount stops the ramp).
export const wavesShader: ShaderDefinition = {
  frag: FRAG,
  colors: [
  ...
  colorCount: 4,
  shape: [1.54, 0.71, 0.48, 0.126],
  surface: [3.296, 0.996, 0.0, 0.74],
  finish: [1.3788, 0.37, 0.008, 0.091],
  transform: [7578.0, 4.5029, 0.14, 0.0],
  timeScale: 0.86,
};
