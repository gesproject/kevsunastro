...
/**
 * "Perlin Noise" (moss palette) -- supplied verbatim by the human's design
 * reference for the Music section backdrop, adapted from Paper Shaders
 * (Apache-2.0, https://shaders.paper.design/perlin-noise) via the 21st.dev
 * Shader Builder. Ported from its original React/canvas-ref component to
 * this project's shared vanilla WebGL engine (shaderField.ts) -- same
 * shader math and uniform values, no React/Tailwind/shadcn involved, since
 * this project ships zero client React on public routes. The source
 * component's pointer-follow cursor effect is not ported: its own config
 * shipped with cursorEnabled: false, so that code path never ran.
 */
