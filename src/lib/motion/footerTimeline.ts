...
export function initFooterTimeline(): void {
  ...
  const context = gsap.context(() => {
    ...
    media.add(DESKTOP_QUERY, () => {
      gsap.set(booking, { autoAlpha: 0, y: 12 });
      gsap.set(links, { autoAlpha: 0, y: 8 });
      const timeline = gsap.timeline({
        scrollTrigger: { trigger: footer, start: "top 85%", toggleActions: "play none none none" },
      });
      timeline.to(booking, { autoAlpha: 1, y: 0, duration: 0.6, ease: "power3.out" }, 0);
      timeline.to(links, { autoAlpha: 1, y: 0, stagger: 0.05, duration: 0.45, ease: "power2.out" }, 0.2);
    });
